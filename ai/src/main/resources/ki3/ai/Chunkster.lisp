...
(chunk-type todo state tile flag xCoordinate yCoordinate currRegSlot currHandSlot currIndex)
...
(P goal-tile-not-found "Keine Zielflagge vorhanden: Steuere Flagge 1 an"
	=goal>
		ISA		todo
		state 	find-goal
		flag	nil
...
(P indirect-hole-connection-not-loaded "Verbindung fuert nicht indirekt in kein Loch; fahre normal fort"
	=goal>
		ISA		todo
		state	check-taken-path
	=imaginal>
...
(P calculate-diff-value "Berechne Abstand vom Zielfeld als Zahlenwert, dann leere retrieval-buffer und schaue weitere Karten an; keine gerade Linie zum Zielfeld"
	...
	=retrieval>
		...
		yTurn	=yTurn
		diff	=diff
	!bind! =plus (+ =diff (if (or (eq =turn =xTurn) (eq =turn =yTurn)) (parse-integer "0") (parse-integer "1")))
==>
	=goal>
		state	choose
	=imaginal>
...
(P calculate-diff-value-x "Berechne Abstand vom Zielfeld als Zahlenwert, dann gehe zum Kartenvergleich"
	...
	=retrieval>
		...
		yTurn	=yTurn
		diff	=diff
	!bind! =plus (+ =diff (cond ((eq =yTurn =turn) (read-from-string "0")) ((eq =yTurn =negTurn) (read-from-string "0.5")) (T (read-from-string "0.75"))))
==>
	=goal>
		state	choose
	=imaginal>
...
(P calculate-diff-value-y "Berechne Abstand vom Zielfeld als Zahlenwert, dann gehe zum Kartenvergleich"
	...
	=retrieval>
		...
		yTurn	none
		diff	=diff
	!bind! =plus (+ =diff (cond ((eq =xTurn =turn) (read-from-string "0")) ((eq =xTurn =negTurn) (read-from-string "0.5")) (T (read-from-string "0.75"))))
==>
	=goal>
		state	choose
	=imaginal>
...
(P* old-connection-reached-flag-no-lives-left "Wenn die alte Verbindung eine neue Flagge erreicht hat, aber keine Leben mehr verbleiben, pruefe erst, ob dies die letzte Flagge ist"
	=goal>
		ISA			todo
		state		hole
		flag		=flag
...
(P last-flag-reached "Wenn letzte Flagge erreicht, spiele Karte"
	=goal>
		ISA			todo
		state		check-for-last-flag
	=imaginal>
...
(P* last-flag-not-reached "Wenn letzte Flagge nicht erreicht, spiele Karte nicht"
	=goal>
		ISA			todo
		state		check-for-last-flag
		currRegSlot	=regslot
