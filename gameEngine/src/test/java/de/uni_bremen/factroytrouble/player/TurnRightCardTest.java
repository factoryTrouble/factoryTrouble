...
public class TurnRightCardTest {
    ...
    @Mock
    Robot robot4;
    private TurnRightCard card1;
    private TurnRightCard card2;

    @Before
    public void setUp() {

        when(robot1.getOrientation()).thenReturn(Orientation.EAST);
        when(robot2.getOrientation()).thenReturn(Orientation.SOUTH);
        when(robot3.getOrientation()).thenReturn(Orientation.WEST);
        when(robot4.getOrientation()).thenReturn(Orientation.NORTH);
        ...
        card1 = new GameTurnRightCard(10);
        card2 = new GameTurnRightCard(20);
    }
    ...
    public void testExecute() throws Exception {
        ...
        assertTrue(robot1.getOrientation() == Orientation.EAST);

        card1.execute(robot2);
        assertTrue(robot2.getOrientation() == Orientation.SOUTH);

        card1.execute(robot3);
        assertTrue(robot3.getOrientation() == Orientation.WEST);

        card1.execute(robot4);
        assertTrue(robot4.getOrientation() == Orientation.NORTH);
